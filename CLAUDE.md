...
Codex added two separate right-side index badges. Keep them conceptually
separate:

- `completeness-index.js` is generated by `scripts/build-completeness.js` and
  drives the teal `N/9` badge. This is **content density**, not quality. The
  script ignores title/closing slides, scores inner slides by word count, and
  currently promotes raw `4/9` decks to `5/9`.
- `quality-index.js` is a manual editorial map added by Codex. It drives the
  red `Q N/10` badge and contains five easy-to-edit fields for every index
  card: `coverage`, `clarity`, `examples`, `correctness`, `freshness`.
  Each field is `0`, `1`, or `2`; the visible badge is their sum.
