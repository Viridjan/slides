...
- Source links are generated by `scripts/add-official-source-links.js`, not
  ...
  `SOURCES`/`WIKI` maps:
  - In the `pr` and `su` sections, every standard-library, API or spreadsheet
    function taught to learners receives an official documentation reference
    at its first instructional occurrence in each deck. Maintain these entries
    in `FUNCTION_SOURCES`; do not treat control-flow keywords or locally defined
    example functions as documented library functions.
...
| `su02-05-word-docs-avanzato.html` | Word e Docs: funzioni avanzate |
...
| `pr05-02-css-stile-selettori.html` | CSS: stile e selettori |
| `pr05-03-html-link-immagini.html` | Link, immagini e tabelle |
| `pr05-04-html-form-semantica.html` | Form e semantica |
...
left; only title and description live inside the bordered central navigation
box; the four controls stand outside it on the right. Topic chips are hidden
(`.card-topics {display:none}`). Module codes use three fixed typographic
columns: main code, decimal point and subcode. Flat codes such as `HS01` reserve
an invisible decimal-point column so their hypothetical dot aligns vertically
...
right-side synthetic statistics badge. `last-modified-index.js` is generated by
`scripts/build-last-modified.js`. The date measures the last change to visible
educational content in internal slides only. The generator compares normalized
visible text through Git history and ignores title slides, closing slides,
scripts, CSS, page markers, author labels and automatically generated source
footers. An uncommitted deck receives today's date only when that internal
...
A `pre-commit` hook keeps `last-modified-index.js` in sync automatically: any
commit that stages a deck HTML file re-runs `scripts/build-last-modified.js`
and re-stages the result before the commit lands, so no editorial date is
ever committed stale. `.git/hooks/` is not version-controlled, so install it
once per clone (or after `.git` gets recreated):
...
`search-index.js` is not covered by the hook (renaming or adding a deck
changes its indexable text far more than routine edits do, so it stays a
deliberate step). After adding, editing, renaming or removing a published
deck, regenerate it together with the other generated indexes:
...
it after changing visible deck text, headings, bold terms, table headers,
semantic labels, index codes or deck filenames.

The inventory has three distinct levels; do not collapse them into one count:

- `lemma_normalizzato` groups exact normalized wording and records its slide
  count in `occorrenze_lemma`;
- `chiave_semantica` groups conservative linguistic variants such as
  singular/plural or reordered equivalent labels;
- `macrocategoria` groups related subtopics under a broader course subject.

`gruppo_editoriale`, `tipo_accorpamento` and
`occorrenze_gruppo_editoriale` are the canonical fields for reviewing the
course. The original visible wording remains in `testo_originale` and must not
be mechanically replaced merely to make labels identical.

Editorial decisions follow these rules:

- A linguistic variant may share one editorial group while retaining the most
  natural wording on its slide.
- Membership in the same macro-category does **not** by itself mean the slide
  content is duplicated. Keep specific subtopics where they are taught and use
  `blocco_principale` as the reference for the complete treatment.
- When two decks genuinely teach the same material, keep the full explanation
  in the most appropriate block, reduce the other occurrence to its local
  application and add a bare-filename cross-reference to the principal deck.
- Progressions inside one deck, contextual homonyms and structural labels are
  not duplicate topics. Extend `structuralLabels`, `contextDependent`,
  `macroRules` or `macroCanonical` in the generator when a recurring false
  positive or a new course-wide category is discovered.
- Every generated row must have an `esito_revisione` and
  `azione_editoriale`; macro-category rows must also have a non-empty
  `blocco_principale`.

After changing the classification rules, regenerate the CSV and verify the
required fields rather than hand-correcting generated rows.
