...
conceptually separate values into one visible `S N/10` score:

- `completeness-index.js` is generated by `scripts/build-completeness.js` and
  provides the density component. This is **content density**, not quality. The
  script ignores title/closing slides, scores inner slides by word count, and
  currently promotes raw `4/9` decks to `5/9`.
...
native tooltip is newline-separated: synthetic score, density, total quality,
then each quality field on its own line. For decks with both values, the visible
score is `round((round(density/9*10) + quality) / 2)`. Quiz cards do not have
generated `.txt` density scores, so their visible synthetic score equals their
quality score.
